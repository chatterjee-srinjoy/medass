...
export const CONDITIONS: Record<string, Condition> = {
  ...
  lgib: {
    ...
    shortName: 'LGIB',
    findings: [
      { phase: 'primary', label: 'Skin & Pulse', value: 'May be stable OR pale/cool/diaphoretic with rapid weak pulse if volume loss is big' },
      ...
      { phase: 'vitals', label: 'Vitals', value: 'Can be normal — or tachycardia + hypotension when the bleed is large enough to drop perfusion' },
    ],
    keyDiscriminator: 'Bright red rectal bleeding (hematochezia) — treat as hypovolemic shock the moment perfusion tanks',
    impression: 'Lower GI bleed (diverticular / hemorrhoidal)',
    interventions: {
      airway: 'Maintain; suction if vomiting',
      oxygen: 'NC if mild; NRB 10–15 LPM if shocky',
      meds: 'None. Nothing by mouth',
      medIds: ['oxygen'],
      medControl: 'No',
      positioning: 'If shocky: supine (BLOT). If stable: position of comfort',
    },
    shock: 'watch',
    shockNote: 'Same physiology as upper GI bleed — it\'s blood leaving the tank. Mild/diverticular bleeds often look stable on exam; a massive LGIB gets full BLOT (lay flat, high-flow O2, keep warm) + Code 3, same as UGIB.',
    transport: { code: 'Routine if stable; Code 3 (Load and go) if shock', destination: 'Closest ED' },
    confusedWith: [
      { conditionId: 'ugib-ulcer', how: 'Black tarry stool = upper source (digested blood); bright red = usually lower. Either one can make the patient shocky — perfusion decides the urgency, not just the color.' },
      { conditionId: 'ugib-varices', how: 'Bright red blood can also be massive upper bleed (varices) coming out the bottom fast — look for alcohol/liver history and hematemesis.' },
    ],
  },
